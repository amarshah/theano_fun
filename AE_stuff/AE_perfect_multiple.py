...
Use random input partition, multiple reflections and nonlinearity"""
...
from fuel.datasets import MNIST, BinarizedMNIST
...
class FirstLayer(object):
    def __init__(self, input, rescale):
        self.input = input

        self.output = rescale * input
...
class FinalLayer(object):
    def __init__(self, input, n_in, n_batch, logv=None):
        ...
        if logv is None:
            logv = theano.shared(value=numpy.ones([1, n_in]), name='logv')
...
class FullAE(object):
    def __init__(self, rng, input, rescale, n_in, n_batch, d_bucket, n_reflections, activation, activation_deriv):

        self.First = FirstLayer(input, rescale)
...
def train_AE(learning_rate=0.025, n_epochs=1000, batch_size=100, d_bucket=100, n_reflections=3,
             activation=nonlinearity, activation_deriv=nonlinearity_deriv,
             model_save='AE_multiple.pkl', save_freq=10):  
