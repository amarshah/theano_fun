...
#            log_jacobian = log_jacobian + T.alloc(T.nlinalg.trace(w_b), n_batch)
            ...
            W = T.dot(T.transpose(w_b), w_b) + 0.1*T.eye(bucket_size)
...
def train_AE(learning_rate=0.0001, n_epochs=1000, batch_size=100, d_bucket=120,
             activation=nonlinearity, activation_deriv=nonlinearity_deriv,
             model_save='AE_deep.pkl', save_freq=10):  
...
#    x.tag.test_value = numpy.random.rand(100, 28**2).astype('float32') 
    ...
    h3_max = (AE.Hidden3.output**2).sum(axis=1).max()
    h3_max.name = 'h3_max'
    h3_min = (AE.Hidden3.output**2).sum(axis=1).min()
    h3_min.name = 'h3_min'

    h4_max = (AE.Hidden4.output**2).sum(axis=1).max()
    h4_max.name = 'h4_max'
    h4_min = (AE.Hidden4.output**2).sum(axis=1).min()
    h4_min.name = 'h4_min'
...
#        import pdb; pdb.set_trace()
    ...
    monitor = DataStreamMonitoring(variables=[prior_term, log_jacobian, cost,
                                              h1_min, h1_max, h2_min, h2_max,
                                              h3_min, h3_max, h4_min, h4_max],
