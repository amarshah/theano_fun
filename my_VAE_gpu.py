...
index = theano.shared(value=1, name='index')
...
validate_model = theano.function(
    inputs=[],
    outputs=Out(theano.sandbox.cuda.basic_ops.gpu_from_host(cost), borrow=True),
    givens={
...
while(epoch < n_epochs) and (not done_looping):
    epoch = epoch + 1
    train_negll = trainate_model()
    this_train_negll = numpy.mean(valid_negll)
    ...
    for minibatch_index in xrange(n_train_batches):
        index.set_value(minibatch_index)
        ...
        if (iter+1) % valid_freq == 0:
            ...
            if this_valid_negll < best_valid_negll:
                ...
                with open('~/data/lisatmp3/shahamar/VAE_model.pkl', 'w') as f:
                    cPickle.dump(dict, f)
