...
class GaussParamLayer(object):
    def __init__(self, rng, input, n_in, n_out=2, W=None, b=None ):
        ...
        if W is None:
            W_values = numpy.asarray( 
                rng.uniform( 
                    low  = -0.00001,
                    high =  0.00001,
                    # low  = -numpy.sqrt( 6. / (n_in + 2 * n_out) ),
                    # high =  numpy.sqrt( 6. / (n_in + 2 * n_out) ),
                    size = (n_in,2 * n_out)
                ),
                dtype = theano.config.floatX
            )
        ...
        if b is None:
            b_values = numpy.zeros( (2 * n_out,), dtype = theano.config.floatX )
            b = theano.shared( value=b_values, name='b', borrow=True )
        ...
        lin_output = T.dot(input,self.W) + self.b
        self.mu    = lin_output[:, :n_out] 
        self.lognu = lin_output[:, n_out:]
...
def train_VAE(learning_rate = .01, L2_reg=1, n_epochs = 1000,
    ...
    train_set_x.set_value(train_set_x.get_value() > 0.5)
    valid_set_x.set_value(valid_set_x.get_value() > 0.5)
    test_set_x.set_value(test_set_x.get_value() > 0.5)
    ...
    kl_term = -0.5 * (
        1 + encoder.lognu - encoder.mu ** 2 - T.exp(encoder.lognu)).sum(axis=1)
    expectation_term = (
        x * T.log(T.nnet.sigmoid(decoder.mu)) +
        (1 - x) * T.log(1 - T.nnet.sigmoid(decoder.mu))).sum(axis=1)
    cost = (kl_term - expectation_term).mean()

#     cost = -(
#         T.mean(
#             0.5*(encoder.lognu -(encoder.mu**2) -T.exp(encoder.lognu)).sum(axis=1)
#             + (x * T.nnet.softplus(decoder.mu) - (1 - x) * T.nnet.softplus(-decoder.mu)).sum(axis=1)
#         )
...
#     )    
    ...
    gparams = [T.grad(cost,param, disconnected_inputs='ignore') for param in all_params]
