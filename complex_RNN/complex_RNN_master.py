...
def initialize_matrix(n_in, n_out, name, rng, low=None, high=None):   
...
def initialize_vector(n_in, name, rng, low=None, high=None):   
    bin = np.sqrt(6. / (n_in + 1))
    if low is None:
        low = -bin
    if high is None:
        high = bin
...
def do_fft(input):
    [n_b, n_h_times_2] = input.shape
    n_h = n_h_times_2 / 2
    fft_input = T.reshape(input, (n_b, 2, n_h))
    fft_input = fft_input.dimshuffle(0,2,1)
    fft_output = cufft(fft_input) / T.sqrt(n_h)
    fft_output = fft_output.dimshuffle(0,2,1)
    output = T.reshape(fft_output, (n_b, 2*n_h))
    return output

def do_ifft(input):
    [n_b, n_h_times_2] = input.shape
    n_h = n_h_times_2 / 2
    ifft_input = T.reshape(input, (n_b, 2, n_h))
    ifft_input = ifft_input.dimshuffle(0,2,1)
    ifft_output = cuifft(ifft_input) / T.sqrt(n_h)
    ifft_output = ifft_output.dimshuffle(0,2,1)
    output = T.reshape(ifft_output, (n_b, 2*n_h))
    return output

def scale_diag(input, diag):
    [n_b, n_h_times_2] = input.shape
    n_h = n_h_times_2 / 2
    input_re = input[:, :n_h]
    input_im = input[:, n_h:]
...
def times_reflection(input, reflection):
    [n_b, n_h_times_2] = input.shape
    n_h = n_h_times_2 / 2
    input_re = input[:, :n_h]
    input_im = input[:, n_h:]
    reflect_re = reflection[:n_h]
    reflect_im = reflection[n_h:]
...
def complex_RNN(n_input, n_hidden, n_output, scale_penalty, cost_at_each_time):
    ...
    bin = np.sqrt(3. / n_hidden)
    h_0 = initialize_matrix(1, 2*n_hidden, 'h_0', rng, low=-bin, high=bin)
    reflection = initialize_matrix(n_reflect, 2*n_hidden, 'reflection', rng)
    theta = initialize_matrix(n_theta, n_hidden, 'theta', rng)
    scale = initialize_vector(n_hidden, 'scale', rng, low=1., high=1.)
    hidden_bias = initialize_vector(n_hidden, 'hidden_biasscale', rng, low=-0.01, high=0.01)
    
    U = initialize_matrix(2*n_hidden, n_output, 'U', rng)
    out_bias = initialize_vector(n_output, 'out_bias', rng)
   
    RNN_parameters = [h_0, V_re, V_im, hidden_bias, theta, reflection, scale]
    output_parameters = [U, out_bias] 

    x = T.tensor3()
    if cost_at_each_time is True:
        y = T.tensor3()
    else:
        y = T.matrix()
...
#        step1 = do_ifft(step2, n_hidden)
        ...
        dstep5 = dstep6
#        dstep5 = do_ifft(dstep6, n_hidden)
