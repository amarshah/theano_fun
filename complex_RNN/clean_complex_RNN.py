...
def complex_RNN(n_input, n_hidden, n_output):
    ...
    log_phase_bias = theano.shared(np.zeros((2 * n_hidden,), dtype=theano.config.floatX),
                                   name='log_phase_bias')
    reflection = theano.shared(np.asarray(rng.uniform(low=-0.01,
                                                      high=0.01,
                                                      size=(2 * n_hidden, 2)),
                                          dtype=theano.config.floatX), 
                               name='reflection')
    ...
    parameters = [V_re, V_im, hidden_bias, U, out_bias, theta, h_0, reflection]
...
#    Hadamard = sethadamard(n_hidden)
    ...
    def recurrence(x_t, y_t, h_prev, theta, V_re, V_im, hidden_bias, U, out_bias):    
...
#        nonlin_output_im = T.maximum(lin_output_im + hidden_bias.dimshuffle('x', 0), 0.)        
    ...
    non_sequences = [theta, V_re, V_im, hidden_bias, U, out_bias]
...
def main(**kwargs):
    ...
    n_iter = 20000
    learning_rate = np.float32(0.00015)
...
#    train_y = temp.astype('float32')
    ...
    inputs, parameters, cost = complex_RNN(n_input, n_hidden, n_output)
    ...
    for i in xrange(n_iter):
     #   pdb.set_trace()
        ...
        if (i % 100 == 0):
            ...
            print
            pdb.set_trace()
            param_vals = [p.get_value() for p in parameters]
            cPickle.dump(param_vals.append(loss),
                         file('2015-10-23-cumsum-problem-reflections.pkl', 'wb'))
